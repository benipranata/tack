package config

import (
	"os"
	"path/filepath"
	"strings"
	"testing"
)

func writeTemp(t *testing.T, dir, name, content string) string {
	t.Helper()
	p := filepath.Join(dir, name)
	if err := os.WriteFile(p, []byte(content), 0o644); err != nil {
		t.Fatalf("write %s: %v", p, err)
	}
	return p
}

func TestLoad_Valid(t *testing.T) {
	dir := t.TempDir()
	writeTemp(t, dir, "go.mod", "module example.com/foo\n\ngo 1.26.5\n")
	writeTemp(t, dir, "tack.yaml", `
providers:
  - src/provider-01
  - src/provider-02
packages:
  src/iface:
    Iface:
      name: App
      output: app_iface_gen.go
`)

	cfg, err := Load("", dir)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if cfg.ModuleRoot != dir {
		t.Errorf("ModuleRoot = %q, want %q", cfg.ModuleRoot, dir)
	}
	if len(cfg.Providers) != 2 {
		t.Fatalf("Providers = %v, want 2 entries", cfg.Providers)
	}
	ic, ok := cfg.Packages["src/iface"]["Iface"]
	if !ok {
		t.Fatalf("missing packages.src/iface.Iface")
	}
	if ic.Name != "App" || ic.Output != "app_iface_gen.go" || !ic.LocalScan {
		t.Errorf("unexpected InterfaceConfig: %+v", ic)
	}
}

func TestLoad_PackageLevelProvidersKeyRejected(t *testing.T) {
	dir := t.TempDir()
	writeTemp(t, dir, "go.mod", "module example.com/foo\n\ngo 1.26.5\n")
	writeTemp(t, dir, "tack.yaml", `
packages:
  src/iface:
    providers:
      - src/other
    Iface:
      name: App
`)

	_, err := Load("", dir)
	if err == nil {
		t.Fatal("Load: expected error, got nil")
	}
	if !strings.Contains(err.Error(), "packages.src/iface.providers") {
		t.Errorf("error %q does not name the offending key and location", err.Error())
	}
}

func TestLoad_InterfaceLevelProvidersKeyRejected(t *testing.T) {
	dir := t.TempDir()
	writeTemp(t, dir, "go.mod", "module example.com/foo\n\ngo 1.26.5\n")
	writeTemp(t, dir, "tack.yaml", `
packages:
  src/iface:
    Iface:
      name: App
      providers:
        - src/other
`)

	_, err := Load("", dir)
	if err == nil {
		t.Fatal("Load: expected error, got nil")
	}
	if !strings.Contains(err.Error(), "packages.src/iface.Iface.providers") {
		t.Errorf("error %q does not name the offending key and location", err.Error())
	}
}

func TestLoad_UnknownTopLevelKeyRejected(t *testing.T) {
	dir := t.TempDir()
	writeTemp(t, dir, "go.mod", "module example.com/foo\n\ngo 1.26.5\n")
	writeTemp(t, dir, "tack.yaml", `
bogus: true
packages:
  src/iface:
    Iface:
      name: App
`)

	_, err := Load("", dir)
	if err == nil {
		t.Fatal("Load: expected error, got nil")
	}
	if !strings.Contains(err.Error(), `"bogus"`) {
		t.Errorf("error %q does not name the offending key", err.Error())
	}
}

func TestDiscover_WalksUp(t *testing.T) {
	dir := t.TempDir()
	writeTemp(t, dir, "tack.yaml", "packages: {}\n")
	nested := filepath.Join(dir, "a", "b", "c")
	if err := os.MkdirAll(nested, 0o755); err != nil {
		t.Fatal(err)
	}

	found, err := Discover(nested)
	if err != nil {
		t.Fatalf("Discover: %v", err)
	}
	want := filepath.Join(dir, "tack.yaml")
	if found != want {
		t.Errorf("Discover = %q, want %q", found, want)
	}
}

func TestDiscover_NotFound(t *testing.T) {
	dir := t.TempDir()
	if _, err := Discover(dir); err == nil {
		t.Fatal("Discover: expected error, got nil")
	}
}

func TestLoad_ExplicitConfigSkipsDiscovery(t *testing.T) {
	dir := t.TempDir()
	writeTemp(t, dir, "go.mod", "module example.com/foo\n\ngo 1.26.5\n")
	other := writeTemp(t, dir, "other.yaml", `
packages:
  src/iface:
    Iface:
      name: App
`)

	cfg, err := Load(other, "/nonexistent/does/not/matter")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if cfg.Path != other {
		t.Errorf("Path = %q, want %q", cfg.Path, other)
	}
}
