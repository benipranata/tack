...
**Reason**: This requirement's scenarios were written against the `packages.<dir>.<Interface>` nesting,
which no longer exists as valid syntax at all — `packages:` itself is a removed top-level key under the
`targets:` shape, so "a `providers:` key nested under a package's interface entry" can no longer occur
as a distinct case from "the `packages:` key doesn't exist."
**Migration**: Superseded by the new "Schema rejects what the config parser rejects" requirement below,
